"use strict";
/*!
 * @license
 * Copyright 2019 Alfresco Software, Ltd.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
Object.defineProperty(exports, "__esModule", { value: true });
var adf_testing_1 = require("@alfresco/adf-testing");
var contentServicesPage_1 = require("../pages/adf/contentServicesPage");
var adf_testing_2 = require("@alfresco/adf-testing");
var viewerPage_1 = require("../pages/adf/viewerPage");
var acsUserModel_1 = require("../models/ACS/acsUserModel");
var folderModel_1 = require("../models/ACS/folderModel");
var fileModel_1 = require("../models/ACS/fileModel");
var js_api_1 = require("@alfresco/js-api");
var adf_testing_3 = require("@alfresco/adf-testing");
var util_1 = require("../util/util");
var resources = require("../util/resources");
var protractor_1 = require("protractor");
describe('Pagination - returns to previous page when current is empty', function () {
    var loginPage = new adf_testing_1.LoginPage();
    var contentServicesPage = new contentServicesPage_1.ContentServicesPage();
    var paginationPage = new adf_testing_2.PaginationPage();
    var viewerPage = new viewerPage_1.ViewerPage();
    var acsUser = new acsUserModel_1.AcsUserModel();
    var folderModel = new folderModel_1.FolderModel({ 'name': 'folderOne' });
    var parentFolderModel = new folderModel_1.FolderModel({ 'name': 'parentFolder' });
    var fileNames = [];
    var nrOfFiles = 6;
    var nrOfFolders = 5;
    var lastFile = 'newFile6.txt';
    var lastFolderResponse;
    var pngFileUploaded;
    var folderNames = ['t1', 't2', 't3', 't4', 't5', 't6'];
    var itemsPerPage = {
        five: '5',
        fiveValue: 5
    };
    var files = {
        base: 'newFile',
        extension: '.txt'
    };
    var pngFileInfo = new fileModel_1.FileModel({
        'name': resources.Files.ADF_DOCUMENTS.PNG.file_name,
        'location': resources.Files.ADF_DOCUMENTS.PNG.file_location
    });
    beforeAll(function () { return __awaiter(_this, void 0, void 0, function () {
        var uploadActions, folderUploadedModel, parentFolderResponse, i;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    this.alfrescoJsApi = new js_api_1.AlfrescoApiCompatibility({
                        provider: 'ECM',
                        hostEcm: protractor_1.browser.params.testConfig.adf_acs.host
                    });
                    uploadActions = new adf_testing_3.UploadActions(this.alfrescoJsApi);
                    return [4 /*yield*/, this.alfrescoJsApi.login(protractor_1.browser.params.testConfig.adf.adminEmail, protractor_1.browser.params.testConfig.adf.adminPassword)];
                case 1:
                    _a.sent();
                    return [4 /*yield*/, this.alfrescoJsApi.core.peopleApi.addPerson(acsUser)];
                case 2:
                    _a.sent();
                    fileNames = util_1.Util.generateSequenceFiles(1, nrOfFiles, files.base, files.extension);
                    return [4 /*yield*/, this.alfrescoJsApi.login(acsUser.id, acsUser.password)];
                case 3:
                    _a.sent();
                    return [4 /*yield*/, uploadActions.createFolder(folderModel.name, '-my-')];
                case 4:
                    folderUploadedModel = _a.sent();
                    return [4 /*yield*/, uploadActions.createFolder(parentFolderModel.name, '-my-')];
                case 5:
                    parentFolderResponse = _a.sent();
                    i = 0;
                    _a.label = 6;
                case 6:
                    if (!(i < nrOfFolders)) return [3 /*break*/, 9];
                    return [4 /*yield*/, uploadActions.createFolder(folderNames[i], parentFolderResponse.entry.id)];
                case 7:
                    _a.sent();
                    _a.label = 8;
                case 8:
                    i++;
                    return [3 /*break*/, 6];
                case 9: return [4 /*yield*/, uploadActions.createEmptyFiles(fileNames, folderUploadedModel.entry.id)];
                case 10:
                    _a.sent();
                    return [4 /*yield*/, uploadActions.createFolder(folderNames[5], parentFolderResponse.entry.id)];
                case 11:
                    lastFolderResponse = _a.sent();
                    return [4 /*yield*/, uploadActions.uploadFile(pngFileInfo.location, pngFileInfo.name, lastFolderResponse.entry.id)];
                case 12:
                    pngFileUploaded = _a.sent();
                    return [4 /*yield*/, loginPage.loginToContentServicesUsingUserModel(acsUser)];
                case 13:
                    _a.sent();
                    return [4 /*yield*/, contentServicesPage.goToDocumentList()];
                case 14:
                    _a.sent();
                    return [2 /*return*/];
            }
        });
    }); });
    it('[C274710] Should redirect to previous page when current is emptied', function () { return __awaiter(_this, void 0, void 0, function () {
        var _a, _b, list, _c, _d, _e;
        return __generator(this, function (_f) {
            switch (_f.label) {
                case 0: return [4 /*yield*/, contentServicesPage.doubleClickRow(folderModel.name)];
                case 1:
                    _f.sent();
                    return [4 /*yield*/, contentServicesPage.checkAcsContainer()];
                case 2:
                    _f.sent();
                    return [4 /*yield*/, contentServicesPage.waitForTableBody()];
                case 3:
                    _f.sent();
                    return [4 /*yield*/, paginationPage.selectItemsPerPage(itemsPerPage.five)];
                case 4:
                    _f.sent();
                    return [4 /*yield*/, contentServicesPage.checkAcsContainer()];
                case 5:
                    _f.sent();
                    return [4 /*yield*/, contentServicesPage.waitForTableBody()];
                case 6:
                    _f.sent();
                    _a = expect;
                    return [4 /*yield*/, paginationPage.getCurrentItemsPerPage()];
                case 7: return [4 /*yield*/, _a.apply(void 0, [_f.sent()]).toEqual(itemsPerPage.five)];
                case 8:
                    _f.sent();
                    _b = expect;
                    return [4 /*yield*/, contentServicesPage.numberOfResultsDisplayed()];
                case 9: return [4 /*yield*/, _b.apply(void 0, [_f.sent()]).toBe(itemsPerPage.fiveValue)];
                case 10:
                    _f.sent();
                    return [4 /*yield*/, contentServicesPage.getAllRowsNameColumn()];
                case 11:
                    list = _f.sent();
                    return [4 /*yield*/, expect(util_1.Util.arrayContainsArray(list, fileNames.slice(0, 5))).toEqual(true)];
                case 12:
                    _f.sent();
                    return [4 /*yield*/, paginationPage.clickOnNextPage()];
                case 13:
                    _f.sent();
                    return [4 /*yield*/, contentServicesPage.checkAcsContainer()];
                case 14:
                    _f.sent();
                    return [4 /*yield*/, contentServicesPage.waitForTableBody()];
                case 15:
                    _f.sent();
                    _c = expect;
                    return [4 /*yield*/, paginationPage.getCurrentItemsPerPage()];
                case 16: return [4 /*yield*/, _c.apply(void 0, [_f.sent()]).toEqual(itemsPerPage.five)];
                case 17:
                    _f.sent();
                    return [4 /*yield*/, contentServicesPage.getAllRowsNameColumn()];
                case 18:
                    list = _f.sent();
                    return [4 /*yield*/, expect(util_1.Util.arrayContainsArray(list, fileNames.slice(5, 6))).toEqual(true)];
                case 19:
                    _f.sent();
                    return [4 /*yield*/, contentServicesPage.deleteContent(lastFile)];
                case 20:
                    _f.sent();
                    return [4 /*yield*/, contentServicesPage.checkContentIsNotDisplayed(lastFile)];
                case 21:
                    _f.sent();
                    _d = expect;
                    return [4 /*yield*/, paginationPage.getCurrentItemsPerPage()];
                case 22: return [4 /*yield*/, _d.apply(void 0, [_f.sent()]).toEqual(itemsPerPage.five)];
                case 23:
                    _f.sent();
                    _e = expect;
                    return [4 /*yield*/, contentServicesPage.numberOfResultsDisplayed()];
                case 24: return [4 /*yield*/, _e.apply(void 0, [_f.sent()]).toBe(itemsPerPage.fiveValue)];
                case 25:
                    _f.sent();
                    return [4 /*yield*/, contentServicesPage.getAllRowsNameColumn()];
                case 26:
                    list = _f.sent();
                    return [4 /*yield*/, expect(util_1.Util.arrayContainsArray(list, fileNames.slice(0, 5))).toEqual(true)];
                case 27:
                    _f.sent();
                    return [2 /*return*/];
            }
        });
    }); });
    it('[C297494] Should display content when navigating to a non-empty folder not in the first page', function () { return __awaiter(_this, void 0, void 0, function () {
        var _a, _b;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0: return [4 /*yield*/, contentServicesPage.goToDocumentList()];
                case 1:
                    _c.sent();
                    return [4 /*yield*/, contentServicesPage.doubleClickRow(parentFolderModel.name)];
                case 2:
                    _c.sent();
                    return [4 /*yield*/, contentServicesPage.checkAcsContainer()];
                case 3:
                    _c.sent();
                    return [4 /*yield*/, contentServicesPage.waitForTableBody()];
                case 4:
                    _c.sent();
                    return [4 /*yield*/, paginationPage.selectItemsPerPage(itemsPerPage.five)];
                case 5:
                    _c.sent();
                    return [4 /*yield*/, contentServicesPage.checkAcsContainer()];
                case 6:
                    _c.sent();
                    return [4 /*yield*/, contentServicesPage.waitForTableBody()];
                case 7:
                    _c.sent();
                    _a = expect;
                    return [4 /*yield*/, paginationPage.getCurrentItemsPerPage()];
                case 8: return [4 /*yield*/, _a.apply(void 0, [_c.sent()]).toEqual(itemsPerPage.five)];
                case 9:
                    _c.sent();
                    _b = expect;
                    return [4 /*yield*/, contentServicesPage.numberOfResultsDisplayed()];
                case 10: return [4 /*yield*/, _b.apply(void 0, [_c.sent()]).toBe(itemsPerPage.fiveValue)];
                case 11:
                    _c.sent();
                    return [4 /*yield*/, paginationPage.clickOnNextPage()];
                case 12:
                    _c.sent();
                    return [4 /*yield*/, contentServicesPage.checkAcsContainer()];
                case 13:
                    _c.sent();
                    return [4 /*yield*/, contentServicesPage.waitForTableBody()];
                case 14:
                    _c.sent();
                    return [4 /*yield*/, contentServicesPage.doubleClickRow(lastFolderResponse.entry.name)];
                case 15:
                    _c.sent();
                    return [4 /*yield*/, contentServicesPage.checkContentIsDisplayed(pngFileInfo.name)];
                case 16:
                    _c.sent();
                    return [4 /*yield*/, viewerPage.viewFile(pngFileUploaded.entry.name)];
                case 17:
                    _c.sent();
                    return [4 /*yield*/, viewerPage.checkImgViewerIsDisplayed()];
                case 18:
                    _c.sent();
                    return [4 /*yield*/, viewerPage.clickCloseButton()];
                case 19:
                    _c.sent();
                    return [2 /*return*/];
            }
        });
    }); });
});
//# sourceMappingURL=pagination-empty-current-page.e2e.js.map