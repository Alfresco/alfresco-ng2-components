...
then
    ...
    ./scripts/test-e2e-lib.sh -host localhost:4200 -host_sso "$E2E_HOST" -host_identity "$E2E_HOST" -proxy "$E2E_HOST" -u "$E2E_USERNAME" -p "$E2E_PASSWORD" -e "$E2E_EMAIL" --folder content-services --skip-lint --use-dist -b  || exit 1;
fi;
