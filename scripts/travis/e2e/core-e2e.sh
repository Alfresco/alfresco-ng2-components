...
then
    ...
    then
        ...
        ./scripts/test-e2e-lib.sh -host http://localhost:4200 -u "$E2E_USERNAME" -p "$E2E_PASSWORD" -e "$E2E_EMAIL" -host_sso "$E2E_HOST_SSO" -host_bpm "$E2E_HOST_BPM" -host_identity "$E2E_HOST_IDENTITY" -sl -ud -s "$LIST_CLOUD_SPECS"
    else
        echo "no cloud e2e found"
    fi
fi;
