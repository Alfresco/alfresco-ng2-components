...
git clone https://$TOKEN@github.com/Alfresco/adf-app-manager-ui.git $TEMP_GENERATOR_DIR
...
curl -H "Authorization: token $TOKEN" -X POST -d '{"body":"Update generator to use packages version '$VERSION'","head":"'$BRANCH'","base":"development","title":"Update generator to use packages version"}' https://api.github.com/repos/Alfresco/adf-app-manager-ui/pulls
