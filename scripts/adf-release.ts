...
(async () => {
  ...
  const currentLatestVersion = execSync('npm view @alfresco/adf-core@next version')
    .toString()
    .trim();
...
function parseArgs() {
  const parsedArgs = yargs
    ...
    .check((args) => {
      ...
      const registryIsLocalhost = true || registry.hostname === 'localhost';
      if (!args.local) {
