...
describe('SearchFacetChipTabbedComponent', () => {
  ...
  beforeEach(() => {
    ...
    const tabbedFacet: TabbedFacetField = {
        fields: ['field', 'field2'],
        label: 'LABEL',
        facets: {
            field: facet1,
    ...
    component.tabbedFacet = tabbedFacet;
  ...
  function addBucketItem(field: string, displayValue: string) {
    component.tabbedFacet.facets[field].buckets.items.push({ count: 1, label: displayValue, display: displayValue, filterQuery: '' });
    triggerComponentChanges();
  }

    it('should display correct label for tabbed facet', () => {
        const label = fixture.debugElement.query(By.css('.adf-search-filter-ellipsis')).nativeElement.innerText;
