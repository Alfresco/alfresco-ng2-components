...
describe('DocumentList', () => {
    ...
    it('should setup -sites- default columns when currentFolderId is -sites-', () => {
        spyOn(documentList, 'setupDefaultColumns').and.callThrough();
        documentList.currentFolderId = '-sites-';
        documentList.ngOnInit();
        documentList.ngAfterContentInit();

        expect(documentList.setupDefaultColumns).toHaveBeenCalledWith('-sites-');
        expect(documentList.data.getColumns().length).toBe(3);
    });

    it('should setup -mysites- default columns when currentFolderId is -mysites-', () => {
        spyOn(documentList, 'setupDefaultColumns').and.callThrough();
        documentList.currentFolderId = '-mysites-';
        documentList.ngOnInit();
        documentList.ngAfterContentInit();

        expect(documentList.setupDefaultColumns).toHaveBeenCalledWith('-mysites-');
        expect(documentList.data.getColumns().length).toBe(3);
    });

    it('should use the custom layout', () => {
        spyOn(documentList, 'setupDefaultColumns').and.callThrough();
        let columns = [
            {
                title: 'title',
        ...
        documentList.columnList = {columns: <any> columns};
        documentList.ngOnInit();
        documentList.ngAfterContentInit();

        expect(documentList.hasCustomLayout).toBeTruthy();
        expect(documentList.setupDefaultColumns).not.toHaveBeenCalled();
        expect(documentList.data.getColumns().length).toBe(1);
    });

    it('should use the default columns when changing to a currentFolderId that is incompatible with the custom layout', () => {
        spyOn(documentList, 'isCompatibleWithLayout').and.callThrough();
        spyOn(documentList, 'setupDefaultColumns').and.callThrough();
        let columns = [
            {
                title: 'title',
                key: 'incompatibleWithSites',
                cssClass: 'css',
                srTitle: '',
                type: 'text',
                format: ''
        ...
        expect(documentList.isCompatibleWithLayout).toHaveBeenCalled();
        expect(documentList.setupDefaultColumns).toHaveBeenCalledWith('-sites-');

        expect(documentList.data.getColumns().length).toBe(3);
    });

    it('should use the custom columns when changing to a currentFolderId that is compatible with the custom layout', () => {
        spyOn(documentList, 'isCompatibleWithLayout').and.callThrough();
        spyOn(documentList, 'setupDefaultColumns').and.callThrough();
        let columns = [
            {
    ...
    it('should use the default columns upon reload when folderNode is incompatible with the custom layout', () => {
        spyOn(documentList, 'setupDefaultColumns').and.callThrough();
        let columns = [
            {
                title: 'title',
        ...
        expect(documentList.data.getColumns().length).toBe(5);
    });
