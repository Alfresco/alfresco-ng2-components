...
import { ConfigurableFocusTrapFactory, LiveAnnouncer } from '@angular/cdk/a11y';
...
describe('Column Resizing', () => {
    ...
    describe('Keyboard Column Resizing', () => {
        beforeEach(() => {
            dataTableSchema = [
                new ObjectDataColumn({ key: 'id', title: 'ID', draggable: true, resizable: true, width: 200 }),
                new ObjectDataColumn({ key: 'name', title: 'Name', draggable: true, resizable: true, width: 200 })
            ];
            dataTable.data = new ObjectDataTableAdapter([...data], [...dataTableSchema]);
            dataTable.isResizingEnabled = true;
            fixture.detectChanges();
        ...
        it('should set keyboardResizingColumnIndex on resize handle focus', () => {
            expect(dataTable.keyboardResizingColumnIndex).toBe(-1);

            dataTable.onResizeHandleFocus(0);

            expect(dataTable.keyboardResizingColumnIndex).toBe(0);
        });

        it('should reset keyboardResizingColumnIndex on resize handle blur', () => {
            dataTable.keyboardResizingColumnIndex = 0;

            dataTable.onResizeHandleBlur();

            expect(dataTable.keyboardResizingColumnIndex).toBe(-1);
        });

        it('should increase column width by 10px on ArrowRight key', () => {
            const col = dataTable.data.getColumns()[0];
            col.width = 200;
            const event = new KeyboardEvent('keydown', { key: 'ArrowRight' });
            spyOn(event, 'preventDefault');
            spyOn(event, 'stopPropagation');

            dataTable.onResizeHandleKeydown(event, col, 0);

            expect(col.width).toBe(210);
            expect(event.preventDefault).toHaveBeenCalled();
            expect(event.stopPropagation).toHaveBeenCalled();
        });

        it('should increase column width by 10px on ArrowUp key', () => {
            const col = dataTable.data.getColumns()[0];
            col.width = 200;
        ...
        it('should decrease column width by 10px on ArrowLeft key', () => {
            const col = dataTable.data.getColumns()[0];
            col.width = 200;
            ...
            expect(col.width).toBe(190);
        });

        it('should decrease column width by 10px on ArrowDown key', () => {
            const col = dataTable.data.getColumns()[0];
            col.width = 200;
        ...
        it('should increase column width by 50px with Shift+ArrowRight', () => {
            const col = dataTable.data.getColumns()[0];
            col.width = 200;
        ...
        it('should decrease column width by 50px with Shift+ArrowLeft', () => {
            const col = dataTable.data.getColumns()[0];
            col.width = 200;
        ...
        it('should set column width to minimum (100px) on Home key', () => {
            const col = dataTable.data.getColumns()[0];
            col.width = 300;
        ...
        it('should set column width to maximum (500px) on End key', () => {
            const col = dataTable.data.getColumns()[0];
            col.width = 200;
        ...
        it('should not decrease column width below minimum (100px)', () => {
            const col = dataTable.data.getColumns()[0];
            col.width = 105;
            const event = new KeyboardEvent('keydown', { key: 'ArrowLeft' });
        ...
        it('should not increase column width above maximum (500px)', () => {
            const col = dataTable.data.getColumns()[0];
            col.width = 495;
            const event = new KeyboardEvent('keydown', { key: 'ArrowRight' });
        ...
        it('should emit columnsWidthChanged event on keyboard resize', () => {
            spyOn(dataTable.columnsWidthChanged, 'emit');
            const col = dataTable.data.getColumns()[0];
            col.width = 200;
            const event = new KeyboardEvent('keydown', { key: 'ArrowRight' });
        ...
        it('should announce column width change via LiveAnnouncer', () => {
        ...
        it('should update aria-valuenow attribute based on column width', () => {
            const col = dataTable.data.getColumns()[0];
            col.width = 250;
            fixture.detectChanges();

            const resizeHandle = testingUtils.getByCSS('.adf-datatable__resize-handle');

            expect(resizeHandle.attributes['aria-valuenow']).toBe('250');
        });

        it('should show resize handle when focused via keyboard', () => {
            const resizeHandle = testingUtils.getByCSS('.adf-datatable__resize-handle');

            resizeHandle.nativeElement.dispatchEvent(new FocusEvent('focus'));
            fixture.detectChanges();
