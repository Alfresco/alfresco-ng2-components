...
fdescribe('Sites service', () => {
    ...
    it('Should get only the sites where the logged user is member of', async(() => {
        spyOn(service, 'getEcmCurrentLoggedUserName').and.returnValue('test');

        service.getMembershipSites('fake-site-id').subscribe((data) => {
            expect(data.list.entries.length).toBe(2);
            expect(data.list.entries[0].entry.visibility).toBe('PUBLIC');
            expect(data.list.entries[0].entry.title).toBe('FAKE-SITE-PUBLIC');
            expect(data.list.entries[1].entry.visibility).toBe('PRIVATE');
            expect(data.list.entries[1].entry.title).toBe('FAKE-PRIVATE-SITE-MEMBER');
        });

        jasmine.Ajax.requests.mostRecent().respondWith({
            status: 200,
            contentType: 'json',
            responseText: siteListWitMembers
        });
    }));
