...
import { LoginSSOPage, ErrorPage, BrowserActions, ApiService, UserModel, getTestConfig } from '@alfresco/adf-testing';
...
describe('Error Component', () => {
    ...
    it('[C277302] Should display the error 403 when access to unauthorized page - My Change', async () => {
        await BrowserActions.getUrl(testConfig.adf.url + '/error/403');
    ...
    it('[C277304] Should display the error 404 when access to not found page', async () => {
        await BrowserActions.getUrl(testConfig.adf.url + '/error/404');
    ...
    it('[C307029] Should display Unknown message when error is undefined', async () => {
        await BrowserActions.getUrl(testConfig.adf.url + '/error/501');
